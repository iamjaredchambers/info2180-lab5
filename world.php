...
if ($_SERVER["REQUEST_METHOD"] == "GET") {
    ...
    if ( isset($_GET['country'])  && !isset($_GET['context']) && !empty($_GET['country']))  { 
        $search = ucwords(trim(strip_tags($_GET["country"])));
   ...
   <?php  }
   else if ( isset($_GET['country'])  && isset($_GET['context']) && $_GET['context'] == "cities" && !empty($_GET['country']) ){
        $search = ucwords(trim(strip_tags($_GET['country'])));
        $stmt = $conn->query("SELECT cities.name, cities.district, cities.population FROM cities  JOIN 
        countries ON cities.country_code = countries.code  WHERE countries.name LIKE '%$search%' ");
        $results = $stmt->fetchAll(PDO::FETCH_ASSOC);?>
